...
function matchesCustomPattern(filePath: string): boolean {
  ...
  return PROTECTED_PATTERNS.customPatterns.some(pattern => {
    ...
    return matchesWildcardPattern(normalizedPath, pattern.toLowerCase()) ||
...
function handlePreToolUse(event: PreToolUseEvent): PermissionResponse {
  ...
  if (toolName === "Read") {
    ...
    if (pattern.includes(".env") || pattern.includes("secret") || 
        pattern.includes("password") || pattern.includes("key")) {
      logMessage("WARNING", `Blocked Glob pattern targeting sensitive files: ${pattern}`);
      return {
        hookSpecificOutput: {
          hookEventName: "PreToolUse",
    ...
    if (searchPath) {
      ...
      if (protection.isProtected) {
        logMessage("WARNING", `Blocked Grep in protected location: ${searchPath} - ${protection.reason}`);
        return {
          hookSpecificOutput: {
            hookEventName: "PreToolUse",
            permissionDecision: "deny",
